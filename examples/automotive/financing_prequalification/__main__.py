...
class FinancingPrequalificationController(guava.CallController):
    def __init__(self):
        super().__init__()
    ...
    def save_results(self):
        results = {
            ...
            "vehicle_of_interest": self.get_field("vehicle_of_interest"),
            "estimated_purchase_price": self.get_field("estimated_purchase_price"),
            "annual_income": self.get_field("annual_income"),
            "employment_status": self.get_field("employment_status"),
            "down_payment_amount": self.get_field("down_payment_amount"),
            "credit_score_range": self.get_field("credit_score_range"),
            "trade_in_vehicle": self.get_field("trade_in_vehicle"),
        }

        print(json.dumps(results, indent=2))
        logging.info(
            "Financing pre-qualification intake completed for %s",
            self.get_field("full_name"),
        )
...
if __name__ == "__main__":
    ...
    guava.Client().listen_inbound(
        ...
        controller_class=FinancingPrequalificationController,
    )
