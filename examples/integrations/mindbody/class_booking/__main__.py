...
agent = guava.Agent(
    ...
    purpose="to help FlexFit Studio members book fitness classes and manage their schedules",
)
...
def lookup_client(call: guava.Call) -> None:
    ...
    preferred_day = call.get_field("preferred_day")
    ...
    try:
        ...
        if not active_credits:
            logging.info("Client %s has no active class credits.", client_id)
            call.hangup(
                final_instructions=(
                    f"Let {client_name} know that we weren't able to find any active class "
        ...
        logging.info(
            "Client %s has %d active service(s).", client_id, len(active_credits)
        )
    ...
    instructor = "one of our instructors"
    ...
    try:
        ...
        if not candidates:
            ...
            call.hangup(
                final_instructions=(
                    f"Let {client_name} know that we don't have any {class_type} classes "
        ...
        class_name = selected_class.get("ClassDescription", {}).get("Name", class_type)
        instructor = selected_class.get("Staff", {}).get("Name", "one of our instructors")
        start_raw = selected_class.get("StartDateTime", "")
        max_cap = selected_class.get("MaxCapacity", 0)
        total_booked = selected_class.get("TotalBooked", 0)
    ...
    call.set_task(
        ...
        objective=(
            f"Present the found {class_type} class to {client_name} and confirm they "
            "want to book it."
        ),
        checklist=[
            guava.Say(
                f"Great news, {client_name}! I found a {class_name} class on {display_time} "
...
@agent.on_task_complete("confirm_class_booking")
def finalize_booking(call: guava.Call) -> None:
    ...
    if confirm != "yes, book it":
        call.hangup(
            final_instructions=(
                f"No problem — let {client_name} know no class was booked. "
    ...
    try:
        ...
        call.hangup(
            final_instructions=(
                f"Apologize to {client_name} and let them know we ran into a problem "
    ...
    call.hangup(
        final_instructions=(
            f"Confirm to {client_name} that they're all set — their spot in the "
