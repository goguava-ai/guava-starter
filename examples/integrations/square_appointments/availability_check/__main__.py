...
class AvailabilityCheckController(guava.CallController):
    def __init__(self):
        ...
        self.set_persona(
            ...
            agent_name="Taylor",
            agent_purpose="to help Crestwood Wellness clients check available appointment slots",
        )
    ...
    def check_availability(self):
        ...
        preferred_time_of_day = self.get_field("preferred_time_of_day") or "flexible"
        ...
        self._available_slots = filtered[:5]  # Keep up to 5 slots

        logging.info(
            "%d slots after filtering by time-of-day preference '%s'",
            len(self._available_slots), preferred_time_of_day,
        )
        ...
        slot_descriptions = [format_slot(s) for s in self._available_slots]
        slots_text = "; ".join(slot_descriptions)

        # Ask if they want to book now
        self.set_task(
            objective=(
                f"You found {len(self._available_slots)} available {service_type} slot(s) for "
            ...
            on_complete=self.handle_booking_decision,
        )

    def handle_booking_decision(self):
        wants_to_book = (self.get_field("wants_to_book") or "no").lower()
        chosen_slot_text = self.get_field("chosen_slot") or ""
        booking_email = (self.get_field("booking_email") or "").strip().lower()
        booking_name = (self.get_field("booking_name") or "").strip()
        ...
        chosen = self._available_slots[0] if self._available_slots else None
        if chosen_slot_text and self._available_slots:
