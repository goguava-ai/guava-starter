...
agent = guava.Agent(
    ...
    purpose="to help Crestwood Wellness clients book appointments",
)
...
def handle_complete(call: guava.Call) -> None:
    customer_name = (call.get_field("customer_name") or "").strip()
    customer_email = (call.get_field("customer_email") or "").strip().lower()
    customer_phone = (call.get_field("customer_phone") or "").strip()
