...
@agent.on_task_complete("preference_update")
def on_preference_update_done(call: guava.Call) -> None:
    ...
    for key, pref_key in [
        ...
        value = call.get_field(key) or "no change"
        if value in ("yes", "no"):
            updates[pref_key] = value == "yes"

    contact_freq = call.get_field("contact_frequency") or ""
