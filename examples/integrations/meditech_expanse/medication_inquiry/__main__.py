...
@agent.on_task_complete("collect_identity")
def lookup_medications(call: guava.Call) -> None:
    ...
    try:
        ...
        for entry in med_entries:
            ...
            if med_text:
                ...
                active_medications.append(label)

        logging.info(
            "Found %d active medication(s) for patient %s.",
            len(active_medications),
            patient_fhir_id,
        )
    ...
    if active_medications:
        ...
        med_list_spoken = "; ".join(active_medications)
        medications_intro = (
...
@agent.on_task_complete("handle_inquiry")
def handle_inquiry_reason(call: guava.Call) -> None:
    first_name = call.get_field("first_name")
    last_name = call.get_field("last_name")
