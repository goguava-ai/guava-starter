...
@agent.on_task_complete("collect_registration_info")
def save_registration(call: guava.Call) -> None:
    first_name = call.get_field("first_name")
    last_name = call.get_field("last_name")
    dob = call.get_field("date_of_birth")
    address_line = call.get_field("address_line")
    city = call.get_field("city")
    state = call.get_field("state")
    zip_code = call.get_field("zip_code")
    ...
    insurance_group_number = call.get_field("insurance_group_number")
    ...
    try:
        ...
        if entries:
            existing_patient_id = entries[0]["resource"]["id"]
            logging.info(
                "Existing Meditech patient found: %s", existing_patient_id
            )
    ...
    if existing_patient_id:
        ...
        patient_resource["id"] = existing_patient_id
        try:
            resp = requests.put(
                f"{FHIR_BASE_URL}/Patient/{existing_patient_id}",
                headers=get_headers(),
