...
class DischargeFollowupController(guava.CallController):
    def __init__(self, patient_name: str, patient_id: str, discharge_date: str):
        super().__init__()
        self.patient_name = patient_name
        self.patient_id = patient_id
        self.discharge_date = discharge_date
        self.encounter_id = None
        self.encounter_summary = "your recent hospital stay"
    ...
    def begin_followup(self):
        ...
        try:
            ...
            if entries:
                ...
                self.encounter_id = enc.get("id")
                reason_list = enc.get("reasonCode", [])
                reason_text = reason_list[0].get("text") if reason_list else None
                self.encounter_summary = reason_text or "your recent hospital stay"
                logging.info(
                ...
                logging.warning(
                    "No finished encounters found for patient %s.", self.patient_id
                )
        ...
        self.set_task(
            objective=(
                f"Conduct a post-discharge wellness check with {self.patient_name}, discharged "
                f"from St. Raphael Medical Center on {self.discharge_date}. Collect their pain "
                "level, medication adherence, and any concerning symptoms. Escalate immediately "
                "if they report alarming symptoms."
            ),
            checklist=[
                guava.Say(
                    f"Hi {self.patient_name}, this is Riley calling from "
                    f"St. Raphael Medical Center. We are following up on your discharge on "
                    f"{self.discharge_date} related to {self.encounter_summary}. "
            ...
            on_complete=self.process_and_close,
        )

    def process_and_close(self):
        pain_level = self.get_field("pain_level")
        medication_adherence = self.get_field("medication_adherence")
        concerning_symptoms = self.get_field("concerning_symptoms")
        symptom_description = self.get_field("symptom_description")
        followup_scheduled = self.get_field("followup_scheduled")
        ...
        if escalate:
            self._escalate_to_care_team(pain_score, symptom_description)
            self.hangup(
                final_instructions=(
                    f"Express genuine concern for {self.patient_name}'s wellbeing. "
        ...
        self.hangup(
            final_instructions=(
                f"Thank {self.patient_name} warmly for taking the time to speak with us today. "
                f"{extra} "
    ...
    def _escalate_to_care_team(self, pain_score: int, symptom_description: str):
    ...
    def recipient_unavailable(self):
        self.hangup(
            final_instructions=(
                f"Leave a brief, caring voicemail for {self.patient_name} on behalf of "
                "St. Raphael Medical Center. Let them know we called to check on their "
                f"recovery following their discharge on {self.discharge_date}. "
...
if __name__ == "__main__":
    ...
    guava.Client().create_outbound(
        ...
        call_controller=DischargeFollowupController(
