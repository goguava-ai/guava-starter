...
class ShippingRateInquiryController(guava.CallController):
    def __init__(self):
        super().__init__()
        self.shipment = None
        ...
        self.set_persona(
            ...
            agent_name="Riley",
            agent_purpose="to help customers get shipping rate quotes before sending a package",
        )
    ...
    def _parse_float(self, key: str, default: float = 1.0) -> float:
        try:
    ...
    def handle_complete(self):
        from_street = self.get_field("from_street")
        from_city = self.get_field("from_city")
        from_state = self.get_field("from_state")
        from_zip = self.get_field("from_zip")
        to_street = self.get_field("to_street")
        to_city = self.get_field("to_city")
        to_state = self.get_field("to_state")
        to_zip = self.get_field("to_zip")

        weight_oz = self._parse_float("weight_oz", default=16.0)
        length_in = self._parse_float("length_in", default=12.0)
        width_in = self._parse_float("width_in", default=12.0)
        height_in = self._parse_float("height_in", default=6.0)
