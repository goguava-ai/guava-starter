...
@agent.on_task_complete("lookup_order_and_route")
def on_lookup_done(call: guava.Call) -> None:
    ...
    if "lost" in issue_type or "damaged" in issue_type:
        _collect_claim_details(call)
    elif "delayed" in issue_type:
        _handle_delayed(call)
    elif "wrong" in issue_type:
        _collect_wrong_item_details(call)
    else:
        _collect_claim_details(call)
...
@agent.on_task_complete("finalize_claim")
def on_finalize_claim(call: guava.Call) -> None:
    issue_type = call.get_field("issue_type") or "issue"
