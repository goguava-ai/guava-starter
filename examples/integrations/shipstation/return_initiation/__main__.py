...
@agent.on_task_complete("verify_and_create_return")
def on_done(call: guava.Call) -> None:
    order_number = call.get_field("order_number")
    email = call.get_field("email")
    return_reason = call.get_field("return_reason") or ""
    preferred_resolution = call.get_field("preferred_resolution") or "full refund"
    ...
    try:
        ...
        if active:
            shipment = active[0]
            carrier_code = shipment.get("carrierCode", "ups") or "ups"
            service_code = shipment.get("serviceCode", "ups_ground") or "ups_ground"
    except Exception as e:
