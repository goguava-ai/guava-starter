...
class ProductAvailabilityController(guava.CallController):
    def __init__(self):
        ...
        self.set_persona(
            ...
            agent_name="Riley",
            agent_purpose=(
                "to help customers check whether a product is in stock and available to order"
            ),
        )
    ...
    def check_availability(self):
        product_query = self.get_field("product_query") or ""
        have_sku = self.get_field("have_sku")
        sku = (self.get_field("sku") or "").strip()
