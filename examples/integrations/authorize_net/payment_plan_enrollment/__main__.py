...
class PaymentPlanEnrollmentController(guava.CallController):
    def __init__(self):
        super().__init__()
        ...
        self.set_persona(
            ...
            agent_name="Jordan",
            agent_purpose="to help patients enroll in a payment plan for their outstanding balance",
        )
    ...
    def handle_complete(self):
        confirm = self.get_field("confirm_enrollment")
        ...
        monthly_payment = self.get_field("monthly_payment")
        card_number = self.get_field("card_number")
        exp_date = self.get_field("exp_date")
        card_code = self.get_field("card_code")
        billing_address = self.get_field("billing_address")
        billing_city = self.get_field("billing_city")
        billing_state = self.get_field("billing_state")
        billing_zip = self.get_field("billing_zip")
        start_date = self.get_field("start_date")
        total_balance_str = self.get_field("total_balance")
...
if __name__ == "__main__":
    ...
    guava.Client().listen_inbound(
        ...
        controller_class=PaymentPlanEnrollmentController,
    )
