...
class FailedChargeFollowupController(guava.CallController):
    def __init__(
        self,
        customer_name: str,
        customer_profile_id: str,
        ...
        if self.profile_data:
            try:
                ...
                if profiles:
                    self.payment_profile_id = profiles[0].get("customerPaymentProfileId")
            except Exception as e:
                logging.error("Error extracting payment profile ID: %s", e)

        self.set_persona(
            organization_name="Crestview Dental",
            agent_name="Maya",
            agent_purpose="to follow up with patients about a declined payment and offer assistance",
        )
    ...
    def begin_call(self):
        self.set_task(
            objective=(
                f"Reach {self.customer_name} to follow up about a declined charge of "
                f"${self.failed_amount} and offer to retry the payment or update their payment method."
            ),
            checklist=[
                guava.Say(
                ...
                guava.Say(
                    f"Thank you for taking my call, {self.customer_name}. I'm reaching out because "
                    f"a recent payment of ${self.failed_amount} for your account was declined. "
                ...
                guava.Field(
                    ...
                    description=(
                        ...
                        f"${self.failed_amount}. Offer to retry the charge on their card on file now, "
    ...
    def handle_outcome(self):
        resolution = self.get_field("resolution_choice")
        ...
        if resolution == "retry charge now":
            ...
            if error_message:
                self.hangup(
                    final_instructions=(
                        f"Tell {self.customer_name} that unfortunately the charge of ${self.failed_amount} "
                ...
                self.hangup(
                    final_instructions=(
                        f"Tell {self.customer_name} that their payment of ${self.failed_amount} was "
            ...
            self.hangup(
                final_instructions=(
                    f"Thank {self.customer_name} for their time. Let them know that if they need to "
                    f"update their payment method they can call our billing office at (555) 410-2200 "
                    f"or visit our website. Remind them the outstanding balance is ${self.failed_amount}. "
    ...
    def recipient_unavailable(self):
        self.hangup(
            final_instructions=(
                ...
                f"payment of ${self.failed_amount} on their account. Ask them to call us back at "
...
if __name__ == "__main__":
    ...
    guava.Client().create_outbound(
        ...
        call_controller=FailedChargeFollowupController(
