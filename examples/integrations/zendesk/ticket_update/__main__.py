...
class TicketUpdateController(guava.CallController):
    def __init__(self):
        ...
        self.set_task(
            ...
            on_complete=self.verify_ticket,
        )
    ...
    def verify_ticket(self):
        ...
        if self.ticket.get("status") in ("solved", "closed"):
            self.hangup(
                final_instructions=(
                    f"Let {caller_name} know that ticket #{ticket_id} is already "
                    f"{self.ticket['status']} and can no longer be updated. "
    ...
    def post_update(self):
        caller_name = self.get_field("caller_name") or "the caller"
        ticket_id = self.ticket["id"]
        update_text = self.get_field("update_text") or ""
        issue_resolved = self.get_field("issue_resolved") or "still ongoing"
