...
class SeatProvisioningController(guava.CallController):
    def __init__(self):
        super().__init__()
        ...
        self.set_task(
            ...
            on_complete=self.process_seat_request,
        )

        self.accept_call()

    def process_seat_request(self):
        email = (self.get_field("caller_email") or "").strip().lower()
        notes = self.get_field("notes") or ""

        try:
            seats_requested = int(self.get_field("seats_requested") or 0)
...
if __name__ == "__main__":
    ...
    guava.Client().listen_inbound(
        ...
        controller_class=SeatProvisioningController,
    )
