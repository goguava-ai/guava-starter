...
agent = guava.Agent(
    ...
    purpose="to help customers check the status of their support cases with Pinnacle Solutions",
)
...
def _report_case_status(call: guava.Call, case: dict, additional_count: int = 0) -> None:
...
@agent.on_task_complete("lookup_case")
def on_lookup_case(call: guava.Call) -> None:
    method = call.get_field("lookup_method") or "case-number"
    ...
    try:
        if method == "case-number":
            ...
            case = cases[0]
            extra = len(cases) - 1
            _report_case_status(call, case, additional_count=extra)
