...
agent = guava.Agent(
    ...
    purpose="to help customers update their contact information in the Pinnacle Solutions system",
)
...
@agent.on_task_complete("apply_update")
def on_apply_update(call: guava.Call) -> None:
    current_email = (call.get_field("current_email") or "").strip()
    field_to_update = call.get_field("field_to_update") or ""
    new_phone = (call.get_field("new_phone") or "").strip()
    new_email = (call.get_field("new_email") or "").strip()
    new_job_title = (call.get_field("new_job_title") or "").strip()
