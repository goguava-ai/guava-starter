...
class EscalationController(guava.CallController):
    ...
    def escalate(self):
        ...
        ticket_id_raw = (self.get_field("ticket_id") or "").replace("#", "").strip()
        ticket_id = ticket_id_raw if ticket_id_raw.isdigit() else ""
        summary = self.get_field("issue_summary") or "Urgent issue reported by phone"
        business_impact = self.get_field("business_impact") or ""
        urgency_reason = self.get_field("urgency_reason") or "system-down"
