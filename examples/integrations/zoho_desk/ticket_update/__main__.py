...
class TicketUpdateController(guava.CallController):
    def __init__(self):
        super().__init__()
        self.ticket = None
        ...
        self.set_persona(
            ...
            agent_name="Alex",
            agent_purpose="to help customers add information to an existing support ticket",
        )
        ...
        self.set_task(
            ...
            on_complete=self.verify_and_update,
        )

        self.accept_call()

    def verify_and_update(self):
        ticket_id = (self.get_field("ticket_id") or "").replace("#", "").strip()
        caller_email = (self.get_field("caller_email") or "").strip().lower()
        update_details = self.get_field("update_details") or ""
        update_type = self.get_field("update_type") or "new-information"
        ...
        try:
            self.ticket = get_ticket(ticket_id)
        ...
        ticket_contact = self.ticket.get("contact") or {}
        ...
        ticket_status = self.ticket.get("status", "Open")
        ...
        subject = self.ticket.get("subject", "their issue")
        ticket_number = self.ticket.get("ticketNumber") or ticket_id
...
if __name__ == "__main__":
    ...
    guava.Client().listen_inbound(
        ...
        controller_class=TicketUpdateController,
    )
