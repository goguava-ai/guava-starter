...
class AppReloadRequestController(guava.CallController):
    def __init__(self):
        super().__init__()
        ...
        self.set_task(
            ...
            on_complete=self.trigger_and_respond,
        )

        self.accept_call()

    def trigger_and_respond(self):
        identifier = (self.get_field("app_identifier") or "").strip()
        reason = self.get_field("reason") or ""
        confirmed = self.get_field("confirmed") or "yes, trigger it now"
