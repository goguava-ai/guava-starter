...
agent = guava.Agent(
    ...
    purpose=(
        "to confirm a prescription refill and submit the refill "
    ...
    if outcome == "unavailable":
        call.hangup(
            final_instructions=(
                ...
                f"{medication} and asking them to call back to confirm."
            )
        )
        ...
        call.set_task(
            "prescription_refill",
            objective=(
                f"Confirm whether {patient_name} would like to refill their prescription "
                f"for {medication}. Collect their preferred pharmacy and any questions "
                "for the pharmacist or care team."
            ),
            checklist=[
                guava.Say(
                    f"Hi {patient_name}, this is Jordan calling from Cedar Health pharmacy services. "
                    f"I'm reaching out because a refill is available for your prescription of {medication}."
                ),
                guava.Field(
                    key="confirm_refill",
                    description=(
                        f"Ask whether the patient would like to go ahead with the refill of {medication}. "
                        "Capture 'yes' or 'no'."
                    ),
                    field_type="text",
                    required=True,
...
def on_done(call: guava.Call) -> None:
    ...
    medication = call.get_variable("medication")
    confirm_refill = call.get_field("confirm_refill")
    preferred_pharmacy = call.get_field("preferred_pharmacy")
    questions = call.get_field("questions_for_pharmacist")
    ...
    if confirmed:
        ...
        call.hangup(
            final_instructions=(
                f"Acknowledge that {patient_name} does not need the refill at this time. "
